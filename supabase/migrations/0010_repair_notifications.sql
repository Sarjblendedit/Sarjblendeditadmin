-- Recovery for databases where the original notification migration was not run
-- but was later marked as applied during baseline migration reconciliation.
...
begin
  if tg_op = 'INSERT' then
    insert into public.booking_status_history(booking_id, status, note, created_by)
      values(new.id, new.status, 'Booking received', new.customer_id);
    insert into public.notifications(customer_id, booking_id, title, body, type)
      values(new.customer_id, new.id, 'Booking received', 'Your appointment has been received. We will confirm it shortly.', 'booking');
  elsif new.status is distinct from old.status then
    insert into public.booking_status_history(booking_id, status, created_by)
    ...
    message_title := case new.status
      ...
      when 'arrived' then 'Your barber has arrived'
      when 'in_progress' then 'Your grooming session started'
    ...
    insert into public.notifications(customer_id, booking_id, title, body, type)
      values(new.customer_id, new.id, message_title, message_body, 'booking');
  end if;
  return new;
end;
$$;
...
for each row execute procedure public.track_booking_activity();
